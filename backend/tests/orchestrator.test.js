...
test('runAnalysis end-to-end in mock mode produces a complete report', async () => {
  ...
  const id = await orch.createAnalysis({ companyId: 'test_co', productSnapshot: product });
  ...
  const runs = await db.prepare("SELECT agent, status FROM agent_runs WHERE analysis_id = ?").all(id);
